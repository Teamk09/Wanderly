...
import { Itinerary, UserPreferences, ItineraryLocation, GroundingChunk } from '../types';
    ...
    <div className="container mx-auto p-4 md:p-8">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4">
          <ItineraryPlanner onGenerate={handleGenerateItinerary} isLoading={isLoading} />
        </div>
          ...
          <div className="flex flex-col gap-8">
            ...
            {isLoading && (
               <div className="w-full bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg flex flex-col items-center justify-center min-h-[300px] border border-transparent dark:border-gray-700">
                  <LoadingSpinner />
                  <p className="mt-4 text-gray-600 dark:text-gray-300 font-medium">Generating your adventure...</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">This can take up to a minute.</p>
              </div>
            )}
            {error && (
              <div className="w-full bg-red-100 dark:bg-red-900/30 border border-red-400 dark:border-red-500/50 text-red-700 dark:text-red-300 px-4 py-3 rounded-lg" role="alert">
                <strong className="font-bold">Oh no! </strong>
                <span className="block sm:inline">{error}</span>
              </div>
            )}
            {itinerary && <ItineraryDisplay itinerary={itinerary} citations={citations} />}
            {!itinerary && !isLoading && !error && (
              <div className="w-full bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg flex flex-col items-center justify-center min-h-[300px] text-center border border-transparent dark:border-gray-700">
                  ...
                  <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Ready for an Adventure?</h2>
                  <p className="text-gray-600 dark:text-gray-400 mt-2">Fill out the planner on the left to create your personalized itinerary.</p>
              </div>
            )}
          </div>
        </div>
