...
const MapDisplay: React.FC<MapDisplayProps> = ({ locations }) => {
  ...
  useEffect(() => {
    ...
    window.gm_authFailure = () => {
      setApiKeyError("Google Maps authentication failed. Please check if the API key is correct, has billing enabled, and is not restricted.");
    };
    ...
    loadScript(scriptSrc)
      ...
      .catch(() => setApiKeyError("Failed to load Google Maps. Please check the API key and network connection."));
  ...
  useEffect(() => {
    ...
    if (isApiLoaded && mapRef.current && !map && typeof window.google !== 'undefined' && typeof window.google.maps !== 'undefined') {
  ...
  useEffect(() => {
    ...
    const geocodePromises = locations.map(location =>
      geocoder.geocode({ address: location.address })
    ...
    Promise.allSettled(geocodePromises)
      .then(results => {
        ...
        const successfulGeocodes = results
          ...
          .map(r => (r as PromiseFulfilledResult<{location: ItineraryLocation, result: any}>).value);
      ...
      {!isApiLoaded && !hasLocations && (
         <div className="w-full h-full flex items-center justify-center p-4">
            <div className="text-center">
                <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">Loading Map...</h3>
            </div>
         </div>
      )}
      {isApiLoaded && !hasLocations && (
        <div className="w-full h-full flex items-center justify-center p-4">
          <div className="text-center">
            <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">Map Will Appear Here</h3>
            <p className="text-sm text-gray-700 dark:text-gray-400 mt-1">Generate an itinerary to see your route.</p>
          </div>
        </div>
      )}
      <div 
